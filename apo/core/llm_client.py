...
UsageRecord = Union[LLMUsage, Dict[str, Any]]


def aggregate_usage(usages: List[UsageRecord]) -> Dict:
    """Aggregate raw LLMUsage objects and already-aggregated usage summaries."""
    if not usages:
    ...
    for usage in usages:
        if isinstance(usage, dict):
            ...
            total_latency_s += usage.get("total_latency_s", 0.0)
        ...
        total_latency_s += usage.latency_s
    ...
    return {
        ...
        "avg_latency_s": round(total_latency_s / total_calls, 3) if total_calls else 0.0,
        "by_model": by_model,
    }

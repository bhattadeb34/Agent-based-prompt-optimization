...
def _merge_usage_summary(total: Dict[str, Any], usage: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for model, stats in usage.get("by_model", {}).items():
        model_totals = total.setdefault("by_model", {}).setdefault(
            model, {"calls": 0, "tokens": 0}
        )
        model_totals["calls"] += stats.get("calls", 0)
        model_totals["tokens"] += stats.get("tokens", 0)

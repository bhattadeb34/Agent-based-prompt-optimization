...
def _merge_usage_summary(base: Dict[str, Any], extra: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Merge an already-aggregated usage dict into another summary."""
    if not extra:
        return base
    ...
    if base.get("total_calls"):
        base["avg_latency_s"] = round(base.get("total_latency_s", 0) / base["total_calls"], 3)

    by_model = base.setdefault("by_model", {})
    for model, stats in extra.get("by_model", {}).items():
        target = by_model.setdefault(
            model,
            {"calls": 0, "prompt_tokens": 0, "completion_tokens": 0, "tokens": 0, "latency_s": 0.0},
        )
        for key in ("calls", "prompt_tokens", "completion_tokens", "tokens", "latency_s"):
            target[key] = target.get(key, 0) + stats.get(key, 0)
...
def run_agentic_mode(
    ...
    aggregated_usages: List[Dict[str, Any]] = []
    meta_advice = ""
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        # Score the strategy that actually generated this epoch's candidates.
        ...
        if critic_usage:
            aggregated_usages.append(critic_usage)
        ...
        if epoch % meta_interval == 0 or epoch == n_epochs:
            ...
            if meta_usage and isinstance(meta_usage, dict):
                aggregated_usages.append(meta_usage)
            if meta_advice:
    ...
    for usage in aggregated_usages:
        _merge_usage_summary(total_usage, usage)

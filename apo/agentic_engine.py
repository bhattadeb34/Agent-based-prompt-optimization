...
def _merge_usage_summaries(*summaries: Optional[Dict]) -> Dict:
    """Merge aggregate_usage-style dictionaries without mixing them into LLMUsage lists."""
    merged: Dict[str, Any] = {
        "total_calls": 0,
    ...
    for summary in summaries:
        if not summary:
            continue
        merged["total_calls"] += summary.get("total_calls", 0)
        merged["total_prompt_tokens"] += summary.get("total_prompt_tokens", 0)
        merged["total_completion_tokens"] += summary.get("total_completion_tokens", 0)
        merged["total_tokens"] += summary.get("total_tokens", 0)
...
def run_agentic_mode(
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        aggregate_usages.append(critic_usage)
        ...
        if epoch % meta_interval == 0 or epoch == n_epochs:
            ...
            if meta_usage:
                aggregate_usages.append(meta_usage)
            if meta_advice:
    ...
    total_usage = _merge_usage_summaries(aggregate_usage(all_usages), *aggregate_usages)

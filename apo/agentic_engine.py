...
def _merge_usage_summary(base: Dict[str, Any], extra: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Merge two aggregate_usage-style dicts without mixing them into LLMUsage lists."""
    if not extra:
        return base
    ...
    base["total_latency_s"] = round(
        base.get("total_latency_s", 0.0) + extra.get("total_latency_s", 0.0),
        3,
    )
    ...
    if base.get("total_calls", 0):
        base["avg_latency_s"] = round(base.get("total_latency_s", 0.0) / base["total_calls"], 3)
    return base
...
def run_agentic_mode(
    ...
    for usage_summary in usage_summaries:

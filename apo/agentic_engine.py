...
def _merge_usage_summary(total: Dict, usage: Optional[Dict]) -> Dict:
    ...
    for model, stats in usage.get("by_model", {}).items():
        model_stats = total.setdefault("by_model", {}).setdefault(model, {"calls": 0, "tokens": 0})
...
def run_agentic_mode(
    ...
    usage_summary: Dict[str, Any] = {"total_calls": 0, "total_tokens": 0, "total_latency_s": 0.0, "by_model": {}}
    meta_advice = ""
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        _merge_usage_summary(usage_summary, aggregate_usage(worker_usages))
        ...
        _merge_usage_summary(usage_summary, critic_usage)
        ...
        if epoch % meta_interval == 0 or epoch == n_epochs:
            ...
            _merge_usage_summary(usage_summary, meta_usage)
            if meta_advice:

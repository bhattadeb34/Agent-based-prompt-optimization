...
def _merge_usage_summaries(*summaries: Optional[Dict]) -> Dict:
    """Merge aggregate_usage() dictionaries without mixing raw LLMUsage objects."""
    merged = {
        "total_calls": 0,
    ...
    merged["avg_latency_s"] = round(
        merged["total_latency_s"] / merged["total_calls"], 3
    ) if merged["total_calls"] else 0.0
    return merged
...
def run_agentic_mode(
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        # Critic evaluates the strategy that produced this epoch's candidates.

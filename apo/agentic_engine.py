...
def _merge_usage_summaries(*summaries: Optional[Dict]) -> Dict:
    """Merge aggregate_usage-style dictionaries, including per-model totals."""
    merged: Dict[str, Any] = {
        "total_calls": 0,
...
def run_agentic_mode(
    ...
    total_usage: Dict[str, Any] = _merge_usage_summaries()
    meta_advice = ""
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        reward = current_state.score if current_state.score is not None else reward_fn.compute(valid_candidates)
        pareto_data = reward_fn.pareto_data([c for c in candidates if c.get("valid")])

        # Log epoch
        # Agentic critic/meta return aggregate dicts; worker returns raw LLMUsage objects.

...
def run_agentic_mode(
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        _merge_usage_dict(aggregate_usage_totals, aggregate_usage(worker_usages))
        ...
        _merge_usage_dict(aggregate_usage_totals, critic_usage)
        ...
        if epoch % meta_interval == 0 or epoch == n_epochs:
            ...
            if meta_usage and isinstance(meta_usage, dict):
                _merge_usage_dict(aggregate_usage_totals, meta_usage)
            if meta_advice:
    ...
    total_usage = aggregate_usage_totals
...
def _merge_usage_dict(total: Dict[str, Any], usage: Optional[Dict[str, Any]]) -> None:
    """Merge an aggregate_usage-style dictionary in place."""
    if not usage:
        return
    ...
    total["total_latency_s"] = round(
        total.get("total_latency_s", 0.0) + usage.get("total_latency_s", 0.0),
        3,
    )

...
def run_agentic_mode(
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        # Score the strategy that produced this batch before asking the critic
    ...
    for usage_summary in aggregate_usage_summaries:
        _merge_usage_summary(total_usage, usage_summary)
...
def _merge_usage_summary(target: Dict[str, Any], source: Dict[str, Any]) -> None:
    """Merge an aggregate usage dict into another aggregate usage dict in place."""
    ...
    for key in ("total_latency_s",):
        target[key] = round(target.get(key, 0.0) + source.get(key, 0.0), 3)

    source_by_model = source.get("by_model", {}) or {}
    if not source_by_model:
        return

    target_by_model = target.setdefault("by_model", {})
    for model, stats in source_by_model.items():

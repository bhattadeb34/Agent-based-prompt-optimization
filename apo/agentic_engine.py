...
def _merge_usage_summary(base: Dict[str, Any], extra: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Merge aggregate usage dictionaries without mixing them with LLMUsage objects."""
    if not extra:
        return base
    ...
    for model, stats in extra.get("by_model", {}).items():
        model_stats = base.setdefault("by_model", {}).setdefault(model, {"calls": 0, "tokens": 0})
        model_stats["calls"] += stats.get("calls", 0)
        model_stats["tokens"] += stats.get("tokens", 0)

    calls = base.get("total_calls", 0)
    if calls:
        base["avg_latency_s"] = round(base.get("total_latency_s", 0.0) / calls, 3)
        base["total_latency_s"] = round(base.get("total_latency_s", 0.0), 3)
    return base
...
def run_agentic_mode(
    ...
    total_usage: Dict[str, Any] = {"total_calls": 0, "total_tokens": 0, "total_latency_s": 0.0, "by_model": {}}
    meta_advice = ""
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        total_usage = _merge_usage_summary(total_usage, epoch_usage)
        ...
        if epoch % meta_interval == 0 or epoch == n_epochs:
            ...
            if meta_usage and isinstance(meta_usage, dict):
                total_usage = _merge_usage_summary(total_usage, meta_usage)
            if meta_advice:

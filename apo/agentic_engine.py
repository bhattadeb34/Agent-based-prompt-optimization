...
def _merge_usage_summary(base: Dict[str, Any], extra: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Merge aggregate_usage() dictionaries without mixing them with LLMUsage objects."""
    if not extra:
        return base
    ...
    for key, value in extra.items():
        if key == "by_model" and isinstance(value, dict):
            ...
            for model, stats in value.items():
                current = dict(by_model.get(model, {}))
...
def run_agentic_mode(
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        # Score the strategy that produced this epoch's candidates before the
        # critic creates the next strategy.

...
def run_agentic_mode(
    ...
    for usage_summary in aggregate_usage_summaries:
        total_usage["total_calls"] = total_usage.get("total_calls", 0) + usage_summary.get("total_calls", 0)
        total_usage["total_tokens"] = total_usage.get("total_tokens", 0) + usage_summary.get("total_tokens", 0)
        total_usage["total_prompt_tokens"] = (
            total_usage.get("total_prompt_tokens", 0) + usage_summary.get("total_prompt_tokens", 0)
        )
        total_usage["total_completion_tokens"] = (
            total_usage.get("total_completion_tokens", 0) + usage_summary.get("total_completion_tokens", 0)
        )
        total_usage["total_latency_s"] = round(
            total_usage.get("total_latency_s", 0.0) + usage_summary.get("total_latency_s", 0.0),
            3,
        )
        by_model = total_usage.setdefault("by_model", {})
        for model, stats in usage_summary.get("by_model", {}).items():

...
def _merge_usage_summary(target: Dict, summary: Optional[Dict]) -> Dict:
    """Merge an aggregate usage summary dict into another summary dict."""
    if not summary:
        return target
    ...
    for model, stats in summary.get("by_model", {}).items():
        model_stats = target.setdefault("by_model", {}).setdefault(
            model,
            {"calls": 0, "tokens": 0},
        )
    ...
    if target.get("total_calls"):
        target["avg_latency_s"] = round(
            target.get("total_latency_s", 0.0) / target["total_calls"],
            3,
        )
    return target

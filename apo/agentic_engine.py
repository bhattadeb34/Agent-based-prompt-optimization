...
def _merge_usage_summary(target: Dict, extra: Optional[Dict]) -> Dict:
    """Merge an aggregate_usage() summary into another summary dict in-place."""
    if not extra:
        return target

    for key in ("total_calls", "total_prompt_tokens", "total_completion_tokens", "total_tokens"):
        target[key] = target.get(key, 0) + extra.get(key, 0)
    target["total_latency_s"] = round(
        target.get("total_latency_s", 0.0) + extra.get("total_latency_s", 0.0),
        3,
    )

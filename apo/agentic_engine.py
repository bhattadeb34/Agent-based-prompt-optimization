...
def run_agentic_mode(
    ...
    total_usage: Dict[str, Any] = _empty_usage_summary()
    meta_advice = ""
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        _merge_usage_summary(total_usage, aggregate_usage(worker_usages))
        ...
        _merge_usage_summary(total_usage, critic_usage)
...
def _merge_usage_summary(target: Dict[str, Any], source: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Merge aggregate_usage-style dictionaries in place."""
    if not source:
        return target
    ...
    target["total_latency_s"] = round(
        target.get("total_latency_s", 0.0) + source.get("total_latency_s", 0.0),
        3,
    )

    target_by_model = target.setdefault("by_model", {})
    for model, stats in source.get("by_model", {}).items():
        model_stats = target_by_model.setdefault(model, {"calls": 0, "tokens": 0})
        model_stats["calls"] += stats.get("calls", 0)
        model_stats["tokens"] += stats.get("tokens", 0)

    total_calls = target.get("total_calls", 0)
    if total_calls:
        target["avg_latency_s"] = round(target.get("total_latency_s", 0.0) / total_calls, 3)
    return target

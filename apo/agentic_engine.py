...
def _merge_usage_dicts(*usage_dicts: Optional[Dict]) -> Dict:
    """Merge aggregated LLM usage dictionaries."""
    merged: Dict[str, Any] = {
        "total_calls": 0,
...
def run_agentic_mode(
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        total_usage = _merge_usage_dicts(total_usage, worker_usage)
        ...
        total_usage = _merge_usage_dicts(total_usage, critic_usage)

        print(f"[Critic] Refined strategy to v{new_state.version}")

        # CriticAgent scores the strategy that produced these candidates.

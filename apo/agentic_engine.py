...
def _merge_usage_summary(total: Dict[str, Any], addition: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Merge aggregated LLM usage dicts produced by different agents."""
    if not addition:
        return total

    total["total_calls"] = total.get("total_calls", 0) + addition.get("total_calls", 0)
    total["total_prompt_tokens"] = total.get("total_prompt_tokens", 0) + addition.get("total_prompt_tokens", 0)
    total["total_completion_tokens"] = (
        total.get("total_completion_tokens", 0) + addition.get("total_completion_tokens", 0)
    )
    total["total_tokens"] = total.get("total_tokens", 0) + addition.get("total_tokens", 0)
    total["total_latency_s"] = round(
        total.get("total_latency_s", 0.0) + addition.get("total_latency_s", 0.0), 3
    )

    total_by_model = total.setdefault("by_model", {})
    for model, stats in addition.get("by_model", {}).items():
        model_stats = total_by_model.setdefault(model, {"calls": 0, "tokens": 0})
        model_stats["calls"] += stats.get("calls", 0)
        model_stats["tokens"] += stats.get("tokens", 0)

    calls = total.get("total_calls", 0)
    total["avg_latency_s"] = round(total.get("total_latency_s", 0.0) / calls, 3) if calls else 0.0
    return total
...
def run_agentic_mode(
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        _merge_usage_summary(epoch_usage, critic_usage)
        _merge_usage_summary(total_usage, epoch_usage)
        ...
        if epoch % meta_interval == 0 or epoch == n_epochs:
            ...
            _merge_usage_summary(total_usage, meta_usage)
            if meta_advice:

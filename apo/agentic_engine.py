...
def _merge_usage_summary(total: Dict[str, Any], update: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Merge aggregate_usage-style dictionaries, including nested per-model stats."""
    if not update:
        return total
    ...
    for model, stats in update.get("by_model", {}).items():
    ...
    if calls:
        total["avg_latency_s"] = round(total.get("total_latency_s", 0.0) / calls, 3)
    return total
...
def run_agentic_mode(
    ...
    total_usage = _empty_usage_summary()
    meta_advice = ""
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        _merge_usage_summary(total_usage, worker_usage_summary)
        ...
        epoch_usage = _empty_usage_summary()
        _merge_usage_summary(epoch_usage, worker_usage_summary)

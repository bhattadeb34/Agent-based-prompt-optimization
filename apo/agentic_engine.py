...
def run_agentic_mode(
    ...
    aggregate_usages: List[Dict[str, Any]] = []
    meta_advice = ""
    ...
    for usage_summary in aggregate_usages:
...
def _merge_usage_summary(target: Dict[str, Any], source: Dict[str, Any]) -> Dict[str, Any]:
    """Merge an aggregate_usage() summary into another summary in-place."""
    ...
    if target.get("total_calls"):
        target["avg_latency_s"] = round(target.get("total_latency_s", 0.0) / target["total_calls"], 3)
    return target

...
def _merge_usage_dicts(*usages: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Merge pre-aggregated LLM usage dictionaries without assuming LLMUsage objects."""
    merged: Dict[str, Any] = {
        "total_calls": 0,
    ...
    for usage in usages:
        ...
        merged["total_prompt_tokens"] += usage.get("total_prompt_tokens", 0)
        merged["total_completion_tokens"] += usage.get("total_completion_tokens", 0)
        merged["total_tokens"] += usage.get("total_tokens", 0)
        merged["total_latency_s"] += usage.get("total_latency_s", 0.0)
    ...
    merged["avg_latency_s"] = (
        round(merged["total_latency_s"] / merged["total_calls"], 3)
        if merged["total_calls"]
        else 0.0
    )
    if not merged["by_model"]:
        merged.pop("by_model")
    return merged
...
def run_agentic_mode(
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        # Score the strategy that produced these candidates before asking the
        # critic to propose the next strategy.
        ...
        # Log epoch
        epoch_usage = _merge_usage_dicts(worker_usage, critic_usage)
        total_usage = _merge_usage_dicts(total_usage, epoch_usage)
        ...
        if epoch % meta_interval == 0 or epoch == n_epochs:
            ...
            total_usage = _merge_usage_dicts(total_usage, meta_usage)
            if meta_advice:

...
def _merge_usage_summary(total: Dict[str, Any], extra: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Merge an aggregated usage dict into another aggregated usage dict."""
    if not extra:
        return total
    ...
    for key in (
        ...
        total[key] = total.get(key, 0) + extra.get(key, 0)
...
def run_agentic_mode(
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if critic_usage:
            aggregate_summaries.append(critic_usage)
        ...
        if epoch % meta_interval == 0 or epoch == n_epochs:
            ...
            if meta_usage and isinstance(meta_usage, dict):
                aggregate_summaries.append(meta_usage)
            if meta_advice:
    ...
    for usage_summary in aggregate_summaries:

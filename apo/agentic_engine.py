...
def _merge_usage_summary(total: Dict[str, Any], addition: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Merge aggregate LLM usage dicts without mixing them with LLMUsage objects."""
    if not addition:
        return total
    ...
    total["total_latency_s"] = round(
        total.get("total_latency_s", 0.0) + addition.get("total_latency_s", 0.0),
        3,
    )
    ...
    if total_calls:
        total["avg_latency_s"] = round(total.get("total_latency_s", 0.0) / total_calls, 3)
    return total
...
def run_agentic_mode(
    ...
    aggregate_usage_summaries: List[Dict[str, Any]] = []
    meta_advice = ""
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        # Score the strategy that generated these candidates before asking the
        # critic to refine it; the new strategy has not been evaluated yet.
        valid_candidates = [c for c in candidates if c.get("valid")]
        reward = reward_fn.compute(valid_candidates)
        pareto_data = reward_fn.pareto_data(valid_candidates)

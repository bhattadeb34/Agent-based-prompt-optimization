...
def _merge_usage_summaries(base: Dict[str, Any], extra: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Merge aggregate usage dictionaries produced by agentic sub-agents."""
    if not extra:
        return base
...
def run_agentic_mode(
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        _merge_usage_summaries(total_usage, worker_usage)
        ...
        _merge_usage_summaries(total_usage, critic_usage)
        ...
        # Log epoch
        epoch_usage = worker_usage
        _merge_usage_summaries(epoch_usage, critic_usage)
        ...
        if epoch % meta_interval == 0 or epoch == n_epochs:
            ...
            if meta_usage and isinstance(meta_usage, dict):
                _merge_usage_summaries(total_usage, meta_usage)
            if meta_advice:

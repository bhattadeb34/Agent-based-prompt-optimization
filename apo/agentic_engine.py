...
def _merge_usage_summary(total: Dict[str, Any], summary: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Merge an aggregate_usage-style dict into another summary."""
    if not summary:
        return total

    for key in ("total_calls", "total_prompt_tokens", "total_completion_tokens", "total_tokens", "total_latency_s"):
    ...
    if total.get("total_calls"):
        total["total_latency_s"] = round(total.get("total_latency_s", 0.0), 3)
        total["avg_latency_s"] = round(total["total_latency_s"] / total["total_calls"], 3)
    return total
...
def run_agentic_mode(
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        # Score the strategy that generated this epoch's candidates.

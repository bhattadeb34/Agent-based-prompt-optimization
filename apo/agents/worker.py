...
"""
    ...
    def _parse_generation_json(text: str) -> Dict:
        """Parse LLM JSON, tolerating markdown fences or explanatory text."""
        text = text.strip()
        if "```" in text:
            text = re.sub(r"```(?:json)?\n?", "", text).strip()
        try:
    ...
    def _validate_candidates(self, candidates_raw: List[Dict]) -> List[Dict]:
        ...
        for i, (cand, val_result) in enumerate(zip(candidates_raw, validation_results)):
            ...
            cand["valid"] = bool(val_result.get("valid", False)) and marker_ok and child_canonical is not None
            if not cand["valid"]:
                cand["invalid_reason"] = (
                    marker_reason
                    or val_result.get("error")
            ...
            if cand["valid"]:
                try:
                    ...
                    if (
                        ...
                        and abs(cand["child_property"]) > 1e-15
                    ):

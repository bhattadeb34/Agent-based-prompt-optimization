...
"""
    ...
    def _parse_generation_output(text: str) -> List[Dict]:
        """Parse supported worker JSON schemas into flat candidate records."""
        text = text.strip()
        if "```" in text:
            text = re.sub(r"```(?:json)?\n?", "", text).strip()
        try:
        ...
        def add_candidate(parent: str, candidate: Any, fallback_reason: str = "") -> None:
            if isinstance(candidate, str):
                child_smiles = candidate
                explanation = fallback_reason
            elif isinstance(candidate, dict):
                child_smiles = candidate.get("smiles", "")
                explanation = candidate.get("explanation", fallback_reason)
            else:
                return
            candidates.append({
        ...
        if isinstance(generated, dict):
            for parent, payload in generated.items():
                ...
                for idx, child_smiles in enumerate(smiles):
                    reason = reasoning[idx] if idx < len(reasoning) else ""
                    add_candidate(parent, child_smiles, reason)
        elif isinstance(generated, list):
            for parent_entry in generated:
                if not isinstance(parent_entry, dict):
                    continue
                parent = parent_entry.get("parent", "")
                for cand in parent_entry.get("candidates", []):
                    add_candidate(parent, cand)
    ...
    def _validate_candidates(self, candidates_raw: List[Dict]) -> List[Dict]:
        ...
        for cand in candidates_raw:
            ...
            base["parent_smiles"] = parent_key
            base["parent_property"] = parent_value

            try:
                child_value = self.surrogate.predict_single(child_canonical)
            except Exception as e:
                base["invalid_reason"] = f"Prediction failed: {str(e)}"
                validated.append(base)
                continue
            ...
            if parent_value is not None and abs(parent_value) > 1e-15:
                if self.ctx.maximize:
                    improvement = child_value / parent_value
                elif abs(child_value) > 1e-15:
                    improvement = parent_value / child_value
                else:
                    improvement = 0.0
            else:
                improvement = 0.0

...
"""
    ...
    def _validate_candidates(self, candidates_raw: List[Dict]) -> List[Dict]:
        ...
        for i, (cand, val_result) in enumerate(zip(candidates_raw, validation_results)):
            ...
            canonical_child = val_result.get("canonical", child_smiles)
            ...
            if cand["valid"]:
                try:
                    child_value = self.surrogate.predict_single(canonical_child)
                    ...
                    if sim_tool:
                        sim_obs = sim_tool.execute(smiles1=parent_smiles, smiles2=canonical_child)

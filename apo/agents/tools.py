...
class SMILESValidatorTool(Tool):
    ...
    def execute(self, smiles_list: List[str]) -> Observation:
        ...
        for smi in smiles_list:
            valid, reason = validate_smiles(smi, required_markers=self.required_markers)
            if not valid:
                results.append({
                    "smiles": smi,
                    "valid": False,
...
class SimilarityCalculatorTool(Tool):
    ...
    def execute(self, smiles1: str, smiles2: str) -> Observation:
        ...
        if canonicalize(smiles1) is None or canonicalize(smiles2) is None:
            return Observation(
                success=False,
                result=None,
    ...
    def execute(self, smiles_list: List[str]) -> Observation:
        ...
        try:
            ...
            if len(values) != len(smiles_list):
                values = (values + [None] * len(smiles_list))[:len(smiles_list)]
